...
| `stk` (Synthesis ToolKit) | MIT-artig (STK-Lizenz, s. `stk/LICENSE`) | nein (voll) | Modal-Materialtabellen (ModalBar) → `derived/modal-materials.json`; FM-Instrumentenprofile (TubeBell, Rhodey, Wurley, HevyMetl, BeeThree, FMVoices, PercFlut) → `derived/fm-algorithms.json`. Zahlen extrahiert, kein C++-Code übernommen. |
